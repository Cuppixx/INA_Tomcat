 ...
 */
@WebServlet(name = "urlValidatorP3", value = "/url-validator-p3")
public class UrlValidator extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        String url = ((InputBean)request.getSession().getAttribute("data")).getUrl();
            ...
            if(matcher.matches()) {
                try {
                    ...
                    if(code != 404) {
                        ...
                        response.sendRedirect("org/ina/p3/urlInputAdvanced.jsp?errorMessage=The+Website+doesnt+exist.&input=" + url);
                    }
                } catch (Exception e) {
                    response.sendRedirect("org/ina/p3/urlInputAdvanced.jsp?errorMessage=The+website+doesnt+exist.&input=" + url);
                }
            } else {
                response.sendRedirect("org/ina/p3/urlInputAdvanced.jsp?errorMessage=The+entered+URL+does+not+match+the+expected+format+(www.name.suffix).&input=" + url);
            }
        } else {
            response.sendRedirect("org/ina/p3/urlInputAdvanced.jsp?errorMessage=No+valid+website+address+entered.&input=");
        }
    }
}
